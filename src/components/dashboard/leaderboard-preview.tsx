...
function positionColor(position: number) {
  ...
  if (position === 2) return "bg-zinc-200 text-zinc-700";
    ...
    <div className="rounded-2xl bg-card p-4 ring-1 ring-foreground/10">
      ...
      <ul className="mt-3 flex flex-col gap-1">
        {players.map((player) => (
          ...
          >
            <span
              className={`flex size-6 shrink-0 items-center justify-center rounded-full text-xs font-semibold ${positionColor(
                player.position
              )}`}
            >
              {player.position}
